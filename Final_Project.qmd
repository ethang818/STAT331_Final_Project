...
The Response Values from our regression model have a variability of 0.7733. A higher variance indicates a greater dispersion of the data points from the mean of the response variables. With a variability of 0.7733, our response values have a moderate level of variability from the mean.
A variance of 0.3497 in our fitted values. With such a small variance, this indicates that the model does not explain a significant portion of variability in the response variable.
A variance of 0.4236 indicates that a models predictions are not close to the actual observations, which means that our model may not be the best for representing this data.
